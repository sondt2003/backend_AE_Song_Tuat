...
class AccessService {
  ...
  refreshToken = async ({ refreshToken, user, keyStore }) => {
    ...
    if (refreshToken !== keyStore.refreshToken)
      throw Api401Error("Thông tin shop đã tồn tại");
  ...
  singIn = async ({ email, password }) => {
    ...
    return {
      shop: getInfoData({
        fields: ["_id", "name", "email", "avatar", "address_id","msisdn"],
        object: foundShop,
      }),
      tokens,
    };
  ...
  signUp = async (
    { name, email, password, msisdn, address, latitude, longitude },
    role
  ) => {
    ...
    const newKey = await apiKeyModel.create({
      ...
      permission: role ? [role] : [RoleShop.USER],
    });
